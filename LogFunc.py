...
class FullAdder_8Bit_2Zahlen(LogFunc):
    """
    This class calculates two 8-bit numbers and a carry value
    Input[0] = Carry0 (siehe Schaltbild)
    Input[1..8] = Zahl A 8 Bits
    Input[9..16] = Zahl B 8 Bits
    """
    def __init__(self):
        self.__Sum = [FullAdder(), FullAdder(), FullAdder(), FullAdder(), FullAdder(), FullAdder(), FullAdder(), FullAdder()]
        self.__Carry = OrGate(8)
        self.__Carry.set_name("carry")
        LogFunc.__init__(self, 16, 8)
        self.__Name = "FullAdder_8Bit_2Zahlen"
    ...
    def execute(self):
        ...
        """
        for i in range(1, 9) :
                self.__Sum[i].__Sum[0].set_input(0, self.get_input(i))
                self.__Sum[i].__Sum[0].set_input(1, self.get_input(i+8))
                self.__Sum[i].__Sum[0].execute()

                if i = 1:
                        self.__Sum[i].__Sum[1].set_input(0, self.get_input(0))
                else:
                        self.__Sum[i].__Sum[1].set_input(0, self.__Sum[i-1].get_output(1))

                self.__Sum[i].__Sum[1].set_input(1, self.__Sum[i].__Sum[0].get_output(0))
                self.__Sum[i].__Sum[1].execute()

                self.__Sum[i].__Carry.set_input(0, self.__Sum[i].__Sum[0].get_output(1))
                self.__Sum[i].__Carry.set_input(1, self.__Sum[i].__Sum[1].get_output(1))
                self.__Sum[i].__Carry.execute()

                self.__Sum[i]._set_output(0, self.__Sum[1].get_output(0))
                self.__Sum[i]._set_output(1, self.__Carry.get_output(0))